        ...
        <el-row>
         ...
         <el-col :span="12">
            <el-form-item label="角色">
              <el-select v-model="rolePow" placeholder="请选择">
              <!-- <el-select v-model="roleOptions[form.roleIds]" placeholder="请选择"> -->
...
<script>
import { logisticsUser, getUser, delUser, addUser, updateUser, exportUser, resetUserPwd, changeUserStatus, importTemplate } from "@/api/system/user";
...
  },
 ...
 async created() {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      logisticsUser(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
    ...
    handleUpdate(row) {
      ...
      getUser(userId).then(response => {
        ...
        this.form.birthday = response.userDetail.birthday
        this.postOptions = response.posts;
        this.roleOptions = response.roles;
        this.form.postIds = response.postIds.join('');
